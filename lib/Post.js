...
class Post {
	...
	static feed() {
		return Promise.resolve().then(() => {
			...
			const posts = [].concat(...postsByUser).reverse();
	...
	static get(id) {
		return Promise.resolve().then(() => {
			...
			return db.getAsync("fire:post:" + id).then(data => {
				...
				.then(r => {
					const pr = JSON.parse(JSON.stringify(reacts)); /* clone reacts */
					pr.forEach(react => react.users = []);
					...
					for(let i = 0; i < r.length; i += 2) {
						...
						console.log(pr);
						console.log(pr.find(r => r.id == react))
						pr.find(r => r.id == react).users.push(username);
					}
